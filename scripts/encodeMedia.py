...
## Functions
# Encode media
def encodeMedia(config: ConfigParser):

    # Loop through all media types (3 folders)
    for counter in range(3):
        # Set source and destination dir for relevant content
        if counter == 0:  # Movies
            sourceDirectory         = config["local-media"]["movieSource"]
            destinationDirectory    = config["local-media"]["movieDestination"]
            plexDestination         = config["plex-media"]["plexMovies"]
            tempDirectory           = config["local-media"]["movieTemp"]
            archiveDestination      = config["local-media"]["archive"] + "Movies/"
            movieList               = movies

        elif counter == 1:  # TV Shows
            sourceDirectory         = config["local-media"]["tvSource"]
            destinationDirectory    = config["local-media"]["tvDestination"]
            plexDestination         = config["plex-media"]["plexTV"]
            tempDirectory           = config["local-media"]["tvTemp"]
            archiveDestination      = config["local-media"]["archive"] + "TV/"
            movieList               = shows

        else:  # Other Movies
            sourceDirectory         = config["local-media"]["otherSource"]
            destinationDirectory    = config["local-media"]["otherDestination"]
            plexDestination         = config["plex-media"]["plexOther"]
            tempDirectory           = config["local-media"]["otherTemp"]
            archiveDestination      = config["local-media"]["archive"] + "Other/"
            movieList               = others

        print("Source Directory: " + sourceDirectory)
        print("Destination Directory: " + destinationDirectory)
        print("Plex Destination: " + plexDestination)
        print("Temp Directory: " + tempDirectory)
        print("Archive Destination: " + archiveDestination)
